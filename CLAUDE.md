...
- **Diversions** (migration `023`): a flight can land somewhere other than its scheduled arrival. A
  ...
  authoritative over its last raw ADS-B fix (which can be mid-ocean); `/previous-flights` (the map's
  "Flown" replay) likewise relabels a diverted leg **→ C** and ends its track at C (its raw track trails
  off where coverage was lost); `DivertModal.jsx` is opened
...
| `adsb.js` | `/api/adsb` | `/positions` (live + last-known stale fix), `/trail`, `/actuals` (incl. `divertedTo`), `/previous-flights`, `/flight-track/:legId`, `POST`/`DELETE /legs/:legId/divert` (editor-gated diversion mark / remove). |
...
- **Responsive (mobile/iPad) — fully rolled out:** `hooks/useBreakpoint.js` (phone <768 / tablet 768–1023 / desktop ≥1024, from `lib/breakpoints.js`) drives structural swaps. `components/AppShell.jsx` owns both shells' layout: desktop = existing sidebar (unchanged); iPad-portrait = 64px icon-rail `Sidebar collapsed`; phone = `BottomTabBar` (Calendar/Flights/Quotes/Overview + Menu) + `NavDrawer` (full nav incl. hidden Rate Cards/Maintenance + shell switch + Sign out), nav list shared via `lib/navConfig.js`. Utility CSS in `styles/responsive.css` (`.scroll-x`, phone touch-targets/16px inputs) + spacing/type/`--page-pad` tokens in `index.css`.
  - **Per-page pattern (what's actually used across the page sweep):** pages adapt with inline `isPhone` conditionals — multi-column `gridTemplateColumns` collapse to `'1fr'` on phone; wide `whiteSpace:'nowrap'` tables and overflowing tab strips are wrapped in `.scroll-x` (+ a `minWidth`); flex header/button rows get `flexWrap:'wrap'`. Done for Overview, Aircraft, RateCards, Clients, ClientDetail, Finances, FlightDetail, CrewDetail, Quotes (master/detail stacks), Fleet pages, FuelPrices, Maintenance, CrewCalendar, AssistantPage (phone height uses `100dvh` so the input clears the tab bar), and the `Scheduling`/`TripTabs` tab strips.
  - **Calendar phone view:** `pages/Calendar.jsx` renders `components/CalendarAgenda.jsx` (day-grouped flight cards: status edge, route/⤳divert, tail, ET+UTC times, delay badge, crew, pax; tap → flight detail) on phone by default, with a List/Gantt toggle remembered in `localStorage['cal_agenda_mode']`. Shared leg helpers (`legStateColor`, `arrShown`, `floorDay`, `STATE_COLORS`, `STATUS`) live in `lib/calendarLeg.js` (unit-tested) and are imported by both Calendar and CalendarAgenda. Gantt unchanged on tablet/desktop.
  - **Built but currently UNUSED:** `components/Sheet.jsx` (modal/drawer→full-screen on phone) and `components/ResponsiveTable.jsx` (records→cards, matrix→frozen-first-column scroll) exist from the Tier-0 foundation but the page sweep used the lighter inline pattern above instead — they're available for future adoption (e.g. making `PricingSlideOut` a full-screen Sheet on phone, still deferred).
  - **Invariant: desktop (≥1024px) is unchanged** — every responsive change is gated to `<1024px` (or is an overflow-only `.scroll-x`/`flexWrap`).
