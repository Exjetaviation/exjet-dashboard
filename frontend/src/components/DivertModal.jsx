...
// onClose(), onSaved().
export default function DivertModal({ leg, onClose, onSaved }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px', width: '380px', maxWidth: '92vw', boxShadow: '0 12px 48px rgba(0,0,0,0.6)' }}>
        <h3 style={{ margin: '0 0 4px', fontSize: '16px', color: 'var(--text-primary)' }}>Mark diversion</h3>
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <button onClick={save} disabled={!valid || saving} style={{ padding: '8px 14px', background: valid ? '#f59e0b' : 'var(--bg-secondary)', color: valid ? '#1a1a1a' : 'var(--text-secondary)', border: 'none', borderRadius: '7px', cursor: valid ? 'pointer' : 'default', fontWeight: 600, opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving…' : 'Mark diverted'}</button>
        </div>
      </div>
    </div>
  );
